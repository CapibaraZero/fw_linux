...
pub async fn connect_wifi(Json(payload): Json<WiFiAP>) -> String {
    ...
    if current_ssid.len() != 0 {
        Command::new("sh")
            .arg("-c")
            .arg(std::format!("nmcli c down {}", current_ssid))
            .output()
            .expect("failed to execute process");
        // let current_ssid = String::from_utf8_lossy(&current_ssid_raw.stdout);
    }
    ...
    Command::new("sh")
        .arg("-c")
        .arg(std::format!("nmcli c delete {}", payload.ssid))
        .output()
        .expect("failed to execute process");
...
pub async fn scan_wifi(params: Query<Params>) -> String {
    ...
    let raw_res = Command::new("sh")
        .arg("-c")
        .arg(std::format!(
            "ip link set {} up && jc iw dev {} scan",
            interface,
            interface
        ))
        .output()
...
pub async fn create_ap(Json(payload): Json<WiFiHospot>) -> String {
    ...
    let res = Command::new("sh")
        .arg("-c")
        .arg(std::format!(
            "nmcli d wifi hotspot ifname {} ssid {} password {}",
            payload.interface,
            payload.ssid,
            payload.password
        ))
    ...
    if payload.google {
        if !fs::exists("/etc/nodogsplash/htdocs/.google").unwrap() {
            let google_login_zip = fs::read(std::format!("/{}/google_login_page.zip", process_user)).expect("Can't read google page");
    ...
    let mut log_file = OpenOptions::new()
        ...
        .write(true)
        .truncate(true)
        .open(std::format!("/{}/log.txt", process_user))
        .unwrap();
...
pub async fn enable_monitor_mode(Query(params): Query<Params>) -> Json<AirmonResponse> {
    ...
    let raw_res = Command::new("sh")
        .arg("-c")
        .arg(std::format!(
            "rfkill unblock wifi && airmon-ng start {}",
            interface
        ))
        .output()
        .expect("failed to execute process");
    ...
    let _rename_if = Command::new("sh")
        .arg("-c")
        .arg(std::format!(
            "ip link set {} name {}mon",
            interface,
            interface
        ))
        .output();
...
pub async fn disable_monitor_mode(Query(params): Query<Params>) -> Json<AirmonResponse> {
    ...
    let raw_res = Command::new("sh")
        .arg("-c")
        .arg(std::format!("airmon-ng stop {}", interface))
        .output()
        .expect("failed to execute process");
...
pub async fn get_evilportal_log() -> String {
    let process_user = std::env::var("PROCESS_USER").expect("PROCESS_USER must be set.");
    fs::read_to_string(std::format!("/{}/log.txt", process_user)).expect("Can't read evilportal log")
}